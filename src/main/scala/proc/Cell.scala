...
class Cell(val m: AntSim, val g: Grid, val x: Int, val y: Int, var cell_type: CellType) extends  Displayable {
...
//        }
        color = cell_type.color.copy()
        prevR = color.r
        prevG = color.g
        prevB = color.b
    }

    def changeType(newCellType: CellType): Unit ={
